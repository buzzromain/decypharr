...
func assertNoOrphanSecondaryKeys(t *testing.T, s *Store) {
	t.Helper()

	for _, category := range s.index.Categories() {
		for _, key := range s.FilterByCategory(category) {
			if !s.Exists(key) {
				t.Fatalf("orphan key %q found in category index %q", key, category)
			}
		}
	}

	s.index.mu.RLock()
	providers := make([]string, 0, len(s.index.byProvider))
	for provider := range s.index.byProvider {
		providers = append(providers, provider)
	}
	statuses := make([]string, 0, len(s.index.byStatus))
	for status := range s.index.byStatus {
		statuses = append(statuses, status)
	}
	s.index.mu.RUnlock()

	for _, provider := range providers {
		for _, key := range s.FilterByProvider(provider) {
			if !s.Exists(key) {
				t.Fatalf("orphan key %q found in provider index %q", key, provider)
			}
		}
	}

	for _, status := range statuses {
		for _, key := range s.index.GetByStatus(status) {
			if !s.Exists(key) {
				t.Fatalf("orphan key %q found in status index %q", key, status)
			}
		}
	}
}

func TestStore_CrashDuringWrite_RecoversConsistently(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "store.log")

	s, err := New(Config{DataPath: path, CacheSize: 64, SyncInterval: -1})
	if err != nil {
		t.Fatalf("failed to create store: %v", err)
	}

	if err := s.Put("persisted-before-crash", []byte("value-1"), &EntryMeta{
		Category: "sonarr",
		Provider: "rd",
		Status:   "queued",
	}); err != nil {
	...
	// Simulate abrupt crash: do not call Store.Close (which would sync).
	if err := s.log.file.Close(); err != nil {
		t.Fatalf("abrupt close failed: %v", err)
	}

	recovered, err := New(Config{DataPath: path, CacheSize: 64, SyncInterval: -1})
	if err != nil {
		t.Fatalf("failed to reopen store: %v", err)
	}
	defer recovered.Close()

	value, err := recovered.Get("persisted-before-crash")
	if err != nil {
		t.Fatalf("expected key after crash recovery, got error: %v", err)
	}
	if string(value) != "value-1" {
		t.Fatalf("unexpected recovered value: got=%q", string(value))
	}

	if got := recovered.FilterByCategory("sonarr"); len(got) != 1 || got[0] != "persisted-before-crash" {
		t.Fatalf("unexpected category index after recovery: %v", got)
	}
	if got := recovered.FilterByProvider("rd"); len(got) != 1 || got[0] != "persisted-before-crash" {
		t.Fatalf("unexpected provider index after recovery: %v", got)
	}

	assertNoOrphanSecondaryKeys(t, recovered)
}

func TestStore_WALReplay_RestoresAllEntriesOnReopen(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "store.log")
	...
	if err := s.Put("k1", []byte("v1"), &EntryMeta{Category: "sonarr", Provider: "rd", Status: "queued"}); err != nil {
		t.Fatalf("put k1 failed: %v", err)
	}
	if err := s.Put("k2", []byte("v2"), &EntryMeta{Category: "radarr", Provider: "tb", Status: "done"}); err != nil {
		t.Fatalf("put k2 failed: %v", err)
	}
	if err := s.Put("k3", []byte("v3"), &EntryMeta{Category: "sonarr", Provider: "rd", Status: "done"}); err != nil {
		t.Fatalf("put k3 failed: %v", err)
	}
	if err := s.Put("k2", []byte("v2-updated"), &EntryMeta{Category: "radarr", Provider: "tb", Status: "done"}); err != nil {
		t.Fatalf("overwrite k2 failed: %v", err)
	}
	if err := s.Delete("k3"); err != nil {
		t.Fatalf("delete k3 failed: %v", err)
	}

	// Simulate abrupt crash: close file directly, skip graceful Close/sync path.
	if err := s.log.file.Close(); err != nil {
		t.Fatalf("abrupt close failed: %v", err)
	}
	...
	if recovered.Len() != 2 {
		t.Fatalf("expected 2 live entries after WAL replay, got %d", recovered.Len())
	}

	v1, err := recovered.Get("k1")
	if err != nil || string(v1) != "v1" {
		t.Fatalf("k1 replay mismatch: value=%q err=%v", string(v1), err)
	}
	v2, err := recovered.Get("k2")
	if err != nil || string(v2) != "v2-updated" {
		t.Fatalf("k2 replay mismatch: value=%q err=%v", string(v2), err)
	}
	if recovered.Exists("k3") {
		t.Fatal("k3 should not exist after replay because it was deleted")
	}

	assertNoOrphanSecondaryKeys(t, recovered)
}

func TestStore_WALReplay_PartialRecordHandledSafely(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "store.log")
	...
	if err := s.Put("stable-1", []byte("v1"), &EntryMeta{Category: "sonarr", Provider: "rd", Status: "queued"}); err != nil {
		t.Fatalf("put stable-1 failed: %v", err)
	}
	if err := s.Put("stable-2", []byte("v2"), &EntryMeta{Category: "radarr", Provider: "tb", Status: "done"}); err != nil {
		t.Fatalf("put stable-2 failed: %v", err)
	}
	if _, _, err := s.log.Append("partial-tail", []byte("incomplete"), false, "sonarr", "rd", "queued", "", 0, "torrent", false, 1234); err != nil {
	...
	const truncateBytes int64 = 4
	if info.Size() <= logHeaderSize+truncateBytes {
		t.Fatalf("log too small to truncate safely: size=%d", info.Size())
	}
	if err := s.log.file.Truncate(info.Size() - truncateBytes); err != nil {
		t.Fatalf("truncate failed: %v", err)
	}

	// Simulate abrupt crash after partial WAL write.
	if err := s.log.file.Close(); err != nil {
		t.Fatalf("abrupt close failed: %v", err)
	}
	...
	recovered, err := New(Config{DataPath: path, CacheSize: 64, SyncInterval: -1})
	if err != nil {
		t.Fatalf("reopen after partial record should be safe, got error: %v", err)
	}
	defer recovered.Close()

	v1, err := recovered.Get("stable-1")
	if err != nil || string(v1) != "v1" {
		t.Fatalf("stable-1 missing after partial replay: value=%q err=%v", string(v1), err)
	}
	v2, err := recovered.Get("stable-2")
	if err != nil || string(v2) != "v2" {
		t.Fatalf("stable-2 missing after partial replay: value=%q err=%v", string(v2), err)
	}

	_, err = recovered.Get("partial-tail")
	if err == nil {
		t.Fatal("partial-tail should not be visible after truncated record recovery")
	}
	if !errors.Is(err, io.EOF) && !strings.Contains(err.Error(), "not found") {
		t.Fatalf("unexpected error for partial-tail: %v", err)
	}
...
func TestStore_ForEach_ConcurrentDelete_DoesNotFail(t *testing.T) {
	s := newTestStore(t)
