...
func TestSpeedTest_LatencySuccess(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/account/infos" {
			t.Fatalf("path = %s, want /account/infos", r.URL.Path)
		}
	...
	result := dl.SpeedTest(context.Background())
	...
	if result.Provider != "debridlink" {
		t.Fatalf("provider = %q, want debridlink", result.Provider)
	}
...
func TestSpeedTest_LatencyTransportError(t *testing.T) {
	...
	dl := newTestDLWithAccount(closedURL)
	result := dl.SpeedTest(context.Background())
